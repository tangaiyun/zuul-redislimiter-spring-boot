 ...
 */
package com.tay.zuulredislimiter.core;

import com.tay.zuulredislimiter.ZuulRedisLimiterProperties;
import com.tay.zuulredislimiter.event.RateCheckFailureEvent;
...
public final class RateCheckTaskRunner implements ApplicationContextAware {
    ...
    public boolean checkRun(String rateLimiterKey, TimeUnit timeUnit, int permits) {
        ...
        boolean retVal = true;
        try {
            retVal = checkResult.get(zuulRedisLimiterProperties.getCheckActionTimeout(), TimeUnit.MILLISECONDS);
        }
        catch(Exception e) {
